...
- Notes:
  ...
  - Runtime overview summaries now also surface a current recommended `backend/runtimeClass` pair, so the control plane can suggest `container-guarded` when managed Rust runtime is up, fall back to `container-fast` when only bridge-ready clients exist, and avoid leaving runtime choice implicit
