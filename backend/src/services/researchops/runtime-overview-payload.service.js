...
function buildRuntimeOverviewSummary({ daemons = null, rustDaemon = null, runner = null } = {}) {
  ...
  const summary = {
    ...
    rustHostReady: rustDaemon?.hostReady === true,
    rustContainerReady: rustDaemon?.containerReady === true,
    rustHealthState: String(rustDaemon?.healthState || '').trim().toLowerCase() || 'unknown',
    rustLastFailureReason: String(rustDaemon?.lastFailureReason || '').trim() || null,
