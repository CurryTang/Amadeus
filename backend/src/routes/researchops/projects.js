...
router.post('/projects/:projectId/tree/nodes/:nodeId/bridge-run', async (req, res) => {
  try {
    ...
    const payload = await dispatchBridgeTransport({
      ...
      bridgeRuntime,
      viaDaemon: ({ serverId }) => submitNodeBridgeRunViaDaemon({
        userId,
        serverId,
        projectId: project.id,
        nodeId,
    ...
    return res.status(preflightOnly ? 200 : 202).json(payload);
  } catch (error) {
    if (error.code === 'NODE_BLOCKED') {
      return res.status(409).json({
        ...toErrorPayload(error, 'Node is blocked'),
...
router.get('/projects/:projectId/tree/nodes/:nodeId/bridge-context', async (req, res) => {
  try {
    ...
    if (error.code === 'NODE_NOT_FOUND') return res.status(404).json({ error: error.message });
...
router.getNodeDependencyIds = getNodeDependencyIds;
router.evaluateNodeBlocking = evaluateNodeBlocking;
