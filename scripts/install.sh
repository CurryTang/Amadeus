...
prompt_select "Frontend build/compile location" "Local machine" "Remote server"
frontend_build_choice="${REPLY_CHOICE}"
prompt_select "Backend build/compile location" "Local machine" "Remote server"
backend_build_choice="${REPLY_CHOICE}"
...
backend_compile_on="local"
if [[ "${frontend_build_choice}" == "2" ]]; then
  frontend_compile_on="remote"
fi
if [[ "${backend_build_choice}" == "2" ]]; then
...
FRONTEND_DEPLOY_ON=$( [[ "${mode_choice}" == "2" ]] && echo "local" || echo "remote" )
BACKEND_DEPLOY_ON=$( [[ "${mode_choice}" == "2" ]] && echo "local" || echo "remote" )
